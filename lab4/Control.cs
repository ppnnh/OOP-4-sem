    ...
    internal static class Control
    {
        ...
        public static ListViewItem[] SearchInfo(string Number, SearchType searchType)
        {
            ...
            switch (searchType)
            {
                ...
                case SearchType.Тип_вклада:
                    //чота сделать с этим, но можно ничего не делать, тк это не будет чекать
                    //list = bills.Where(x => x._BillType.ToString().Contains(Number)).Select(x => x);
                    break;
        ...
        public static ListViewItem[] SortInfo(SortType sortType)
        {
            ...
            switch (sortType)
            {
                case SortType.Тип_вклада:
                    //чота сделать с этим, но можно ничего не делать, тк это не будет чекать
                    //list = bills.OrderBy(x => x._BillType).Select(x => x);
                    break;
        ...
        public static void GlobalInfoChange(Label lb)
        {
            lb.Text = $"{DateTime.Now}\nКол-во объектов: {bills.Count}\n";
        }
    }
}
